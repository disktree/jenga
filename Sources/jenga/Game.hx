...
class Game extends iron.Trait {
	...
	public function new() {
		...
		notifyOnInit(() -> {
			...
			#if kha_html5
			Scene.active.world.raw.probe.strength = 0.5; // HACK
			#end
	...
	function update() {
		...
		if (blockDragged != null) {
			...
			if (mouse.wheelDelta != 0) {
				if (mouse.wheelDelta < 0) {
					if (cam.transform.loc.y < ZOOM_MIN)
						cam.transform.loc.y += ZOOM_SPEED;
				} else {
					if (cam.transform.loc.y > ZOOM_MAX)
						cam.transform.loc.y -= ZOOM_SPEED;
				}
